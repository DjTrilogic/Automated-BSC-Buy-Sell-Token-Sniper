...
def foundToken(event):
    print(f"New token found: {json.loads(Web3.to_json(event))}")
    try:
...
def listenForTokens(lastRunTime):
    try:
        ...
        print(f"Error in setting up event filter: {e}")
        traceback.print_exc()
        pass


    print(f"Listening for event : {event_filter}")
    ...
    try:
        ...
        print(f"Error in asynchronous loop: {e}")
        traceback.print_exc()
        pass
    finally:
        try:
            ...
            print(f"Error in recursive call to listenForTokens: {e}")
            traceback.print_exc()
            pass
